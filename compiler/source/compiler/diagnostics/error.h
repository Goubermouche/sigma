...
namespace channel {
	constexpr auto error_templates = make_hash_map(
		...
		// lexer errors
		std::pair{ 2000, "invalid '.' character detected at token start" },
		std::pair{ 2001, "two '_' characters immediately one after another are not allowed" },
		std::pair{ 2002, "invalid number format - cannot declare a number with more that one '.' character" },
		std::pair{ 2003, "invalid number format - cannot declare an unsigned number containing '.' characters" },
		std::pair{ 2004, "invalid number format - floating point number must contain a '.' character" },
		std::pair{ 2005, "invalid unterminated character literal detected" },
		std::pair{ 2006, "invalid unterminated string literal detected" },
		// parser 
		std::pair{ 3000, "unexpected token received (expected '{}', but received '{}' instead)" },
		std::pair{ 3001, "unhandled token received ('{}')" },
		std::pair{ 3002, "unhandled non-numerical token received ('{}')" },
		std::pair{ 3003, "unexpected non-type token received ('{}')" },
		std::pair{ 3004, "cannot apply a unary operator a non-identifier value" },
		// codegen errors
		std::pair{ 4000, "function '{}' has already been defined before" },
		std::pair{ 4001, "function '{}' cannot be found" },
		std::pair{ 4002, "function argument count mismatch for function '{}'" },
