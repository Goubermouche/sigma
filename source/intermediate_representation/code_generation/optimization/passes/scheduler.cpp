...
namespace ir::cg {
	void schedule_pass::apply(code_generator_context& context) {
	...
	void schedule_pass::schedule_early(code_generator_context& context, handle<node> n) {
	...
	void schedule_pass::schedule_late(code_generator_context& context, handle<node> n) {
		// don't revisit already visited nodes
		if (!context.work_list.visit(n)) {
			return;
		}
