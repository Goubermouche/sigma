...
namespace channel {
	acceptation_result codegen_visitor::visit_function_node(function_node& node, const codegen_context& context) {
		...
		if(m_function_registry.contains_function(node.get_function_identifier())) {
			return std::unexpected(error::emit<4000>(node.get_declared_position(), node.get_function_identifier()));
		}
	...
	acceptation_result codegen_visitor::visit_function_call_node(function_call_node& node, const codegen_context& context) {
		...
		// check if it exists
		if (!func) {
			return std::unexpected(error::emit<4001>(node.get_declared_position(), node.get_function_identifier()));
		}
		...
		if(!func->is_variadic() && required_arguments.size() != given_arguments.size()) {
			return std::unexpected(error::emit<4002>(node.get_declared_position(), node.get_function_identifier()));
		}
		...
		for (u64 i = 0; i < required_arguments.size(); i++) {
			...
			acceptation_result argument_result = given_arguments[i]->accept(*this, codegen_context(required_arguments[i].second));
			if(!argument_result.has_value()) {
				return argument_result;
			}

			// cast the given argument to match the required argument's type, if necessary
			argument_values[i] = cast_value(argument_result.value(), required_arguments[i].second, node.get_declared_position());
		}

		// parse variadic arguments
		for (u64 i = required_arguments.size(); i < given_arguments.size(); i++) {
			// get the argument value
			acceptation_result argument_result = given_arguments[i]->accept(*this, {});
			if (!argument_result.has_value()) {
				return argument_result;
			}
			...
			if(argument_type == type(type::base::f32, 0)) {
				// f32 -> f64
				llvm::Value* argument_value_cast = cast_value(argument_value, type(type::base::f64, 0), given_arguments[i]->get_declared_position());
				...
				// i1, i8, i16 -> i32
				llvm::Value* argument_value_cast = cast_value(argument_value, type(type::base::i32, 0), given_arguments[i]->get_declared_position());
		...
		if (return_type.is_pointer() || return_type.get_base() != type::base::empty) {
			return std::make_shared<value>(node.get_function_identifier(), return_type, call_inst);
		}
