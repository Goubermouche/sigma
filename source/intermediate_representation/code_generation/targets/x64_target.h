...
namespace ir::cg {
	...
	enum mod : u8 {
		indirect         = 0, // [rax]
		indirect_disp_8  = 1, // [rax + disp8]
		indirect_disp_32 = 2, // [rax + disp32]
		direct           = 3, // rax
	};
	...
	private:
		...
		void emit(const std::string& fmt, arguments && ...args) {
			auto format_args = std::make_format_args(args...);
			const std::string formatted_string = std::vformat(fmt, format_args);
			utility::console::out << formatted_string;
		}
