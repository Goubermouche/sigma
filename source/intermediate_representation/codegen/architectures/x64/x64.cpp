...
namespace ir {
	...
	void x64_architecture::resolve_stack_usage(codegen_context& context) {
		...
		if(/*target == win64 &&*/ caller_usage > 0 && caller_usage < 4) {
			caller_usage = 4;
		}
	...
	void x64_architecture::emit_function_body(
		...
		for (handle<instruction> inst = context.first; inst; inst = inst->next_instruction) {
			...
			if (
				...
				emit_instruction_2(is_xmm ? instruction::FP_XOR : instruction::XOR, destination, destination, inst->dt, bytecode);
			}
			else if (
					...
					if (inst->out_count == 0) {
						...
						if (ternary || inst->ty == instruction::mov || inst->ty == instruction::FP_MOV) {
							if (out->matches(left) == false) {
								emit_instruction_2(static_cast<instruction::type>(mov_op), out, left, dt, bytecode);
							}
						}
						else {
							emit_instruction_2(inst->ty, out, left, dt, bytecode);
						}
					}
				}
				...
				if (inst->fl & instruction::immediate) {
					...
					emit_instruction_2(inst->ty, out, right, dt, bytecode);
				}
