...
namespace ir::cg::x64 {
	utility::string x64_disassembler::disassemble(
		...
		for(u64 i = 0; i < context.basic_block_order.size(); ++i) {
			const i32 block_index = context.basic_block_order[i];
			handle<node> basic_block = context.work_list.get_item(block_index);
	...
	handle<symbol_patch> x64_disassembler::disassemble_block(
		...
		while(range.start < range.end) {
			...
			if (inst.flags & x64_flags_rep) {
				assembly.append("rep ");
			}

			if (inst.flags & x64_flags_lock) {
				assembly.append("lock ");
			}

			assembly.append("{}", inst.get_mnemonic());

			if (inst.data_type_1 >= sse_ss && inst.data_type_1 <= sse_pd) {
				static const char* type_ids[] = { "ss", "sd", "ps", "pd" };
				assembly.append(type_ids[inst.data_type_1 - sse_ss]);
			}
			...
			for (u8 i = 0; i < 4; i++) {
				if (inst.registers[i].is_valid() == false) {
					if (mem && inst.flags & x64_flags_use_mem_op) {
						if (i > 0) {
							assembly.append(", ");
						}
						...
						if (inst.flags & x64_flags_use_rip_mem) {
							const bool is_label = 
								inst.opcode == 0xE8 || 
								inst.opcode == 0xE9 || 
							...
							if (patch && patch->pos == range.start + inst.length - 4) {
								const handle<symbol> target = patch->target;

								if (target->get_name().empty()) {
									assembly.append("sym{}", target);
								}
								else {
									assembly.append("{}", target->get_name());
								}

								patch = patch->next;
							}
							else {
								const u32 target = range.start + inst.length + inst.memory.displacement;
							...
							assembly.append("{} [", get_type_name(inst.data_type_1));

							if (inst.base != 255) {
								assembly.append("{}", get_register_name(inst.base, qword));
							}

							if (inst.memory.index != 255) {
								assembly.append(
									" + {}*{}", get_register_name(inst.memory.index, qword), 1 << static_cast<u8>(inst.memory.scale_)
								);
							}
					...
					else if (imm && (inst.flags & (x64_flags_immediate | x64_flags_absolute))) {
						if (i > 0) {
							assembly.append(", ");
						}
						...
						if (inst.flags & x64_flags_absolute) {
					...
					if (i > 0) {
						...
						if (inst.flags & x64_flags_two_data_types) {
							assembly.append(
								"{}", get_register_name(inst.registers[i], inst.data_type_2)
							);
					...
					assembly.append(
						"{}", get_register_name(inst.registers[i], inst.data_type_1)
					);
				}
			}
	...
	bool x64_disassembler::disassemble_instruction(
		...
		while (true) {
			...
			switch (op) {
				...
				case 0xF0: inst.flags |= x64_flags_lock; break;
				case 0x66: address_16 = true; break;
				case 0x67: address_32 = true; break;
				case 0xF3: inst.flags |= x64_flags_rep; break;
				case 0xF2: inst.flags |= x64_flags_repne; break;
				case 0x2E: inst.segment = x64_segment::cs; break;
				case 0x36: inst.segment = x64_segment::ss; break;
				case 0x3E: inst.segment = x64_segment::ds; break;
				case 0x26: inst.segment = x64_segment::es; break;
				case 0x64: inst.segment = x64_segment::fs; break;
				case 0x65: inst.segment = x64_segment::gs; break;
	...
	done_prefixing:;
		...
		const bool uses_imm = enc == OP_MI || enc == OP_MI8;
		...
		inst.data_type_1 = dword;

		if (flags & OP_64BIT) {
			// basically forced 64bit
			inst.data_type_1 = qword;
		}
			...
			// ps __     OPCODE
			// pd DATA16 OPCODE
			if (inst.flags & x64_flags_repne) {
				inst.data_type_1 = sse_sd;
			}
			else if (inst.flags & x64_flags_rep) {
				inst.data_type_1 = sse_ss;
			}
			else if (address_16) {
				inst.data_type_1 = sse_ss;
			}
			else {
				inst.data_type_1 = sse_ps;
			}

			inst.flags &= ~(x64_flags_rep | x64_flags_repne);
		}
		else {
			if (rex & 0x8) {
				inst.data_type_1 = qword;
			}
			else if (flags & OP_8BIT) {
				inst.data_type_1 = byte;
			}
			else if (address_16) {
				inst.data_type_1 = word;
			}
		}

		ASSERT(enc != OP_BAD, "bad operation detected");
		...
		if (enc == OP_IMM) {
			...
			inst.flags |= x64_flags_immediate;
		...
		if (enc == OP_REL32) {
			inst.flags |= x64_flags_use_rip_mem;
			inst.flags |= x64_flags_use_mem_op;
			inst.base = -1;
			inst.memory.index = -1;
		...
		if (flags & OP_2DT) {
			if (inst.opcode == 0x0FB6 || inst.opcode == 0x0FB7) {
				inst.flags |= x64_flags_two_data_types;
				inst.data_type_2 = inst.opcode == 0x0FB6 ? byte : word;
			}
			else {
				inst.flags |= x64_flags_two_data_types;
				inst.data_type_2 = dword;
			}
		}
		...
		if (enc != OP_M && !uses_imm) {
			if (enc == OP_MC) {
				inst.flags |= x64_flags_two_data_types;
				inst.data_type_2 = byte;
				inst.registers[rx_slot] = rcx;
			}
			else {
				i8 real_rx = static_cast<i8>((rex & 4 ? 8 : 0) | rx);

				if (rex == 0 && inst.data_type_1 == byte && real_rx >= 4) {
					// use high registers
					real_rx += 16;
				}
		...
		if (uses_imm || op == 0x68 || op == 0x69) {
			if ((enc == OP_MI && inst.data_type_1 == byte) || enc == OP_MI8 || op == 0x68) {
				if (current + 1 > bytecode.get_size()) {
					return false;
				}
	...
	ptr_diff x64_disassembler::disassemble_memory_operand(
		...
		if (rm == rsp) {
			...
			const scale scale = static_cast<cg::scale>((sib >> 6));
			const u8 index = (sib >> 3) & 7;
			const u8 base = (sib & 7);

			const u8 base_gpr = mod != indirect || base != rsp ? ((rex & 1 ? 8 : 0) | base) : -1;
			const u8 index_gpr = mod != indirect || index != rsp ? ((rex & 2 ? 8 : 0) | index) : -1;
			...
			if (mod == 0 && base == rbp) {
				mod = indirect_displacement_32;
			}

			inst.base = base_gpr;
			inst.memory.index = index_gpr;
			inst.memory.scale_ = scale;
		}
		else {
			if (mod == indirect && rm == rbp) {
				...
				inst.memory.index = -1;
				inst.memory.scale_ = scale::x1;
			}
		}

		if (mod == indirect_displacement_8) {
			if (current + 1 > length) {
				return 0;
			}
		...
		else if (mod == indirect_displacement_32) {
			if (current + 4 > length) {
				return 0;
			}
	...
	u64 x64_disassembler::emit_get_label(const codegen_context& context, u32 position) {
	...
	std::array<u16, 256> x64_disassembler::get_first_table() {
		...
		do {                                         \
			first_table[(op) + 0] = OP_MR | OP_8BIT; \
			first_table[(op) + 1] = OP_MR;           \
			first_table[(op) + 2] = OP_RM | OP_8BIT; \
			first_table[(op) + 3] = OP_RM;           \
		} while (false)
		...
		first_table[0x80] = OP_MI8 | OP_8BIT | OP_FAKERX;
		first_table[0x81] = OP_MI | OP_FAKERX;
		first_table[0x83] = OP_MI8 | OP_FAKERX;
		first_table[0x84] = OP_MR | OP_8BIT;
		first_table[0x85] = OP_MR;
		first_table[0x8D] = OP_RM;
		first_table[0x63] = OP_RM | OP_2DT;
		first_table[0x68] = OP_IMM;
		first_table[0x69] = OP_RM;
		first_table[0x90] = OP_0ARY;
		first_table[0x99] = OP_0ARY;
		first_table[0xC3] = OP_0ARY;
		first_table[0xC6] = OP_MI | OP_FAKERX | OP_8BIT;
		first_table[0xC7] = OP_MI | OP_FAKERX;
		first_table[0xC0] = OP_MI8 | OP_FAKERX | OP_8BIT;
		first_table[0xC1] = OP_MI8 | OP_FAKERX;
		first_table[0xD2] = OP_MC | OP_FAKERX | OP_8BIT;
		first_table[0xD3] = OP_MC | OP_FAKERX;
		first_table[0xE8] = OP_REL32;
		first_table[0xE9] = OP_REL32;
		first_table[0xEB] = OP_REL8;
		first_table[0xF6] = OP_M | OP_FAKERX | OP_8BIT;
		first_table[0xF7] = OP_M | OP_FAKERX;
		first_table[0xFF] = OP_M | OP_FAKERX;

		for (int i = 0x50; i <= 0x5F; ++i) {
			first_table[i] = OP_PLUSR | OP_64BIT;
		}

		for (int i = 0x70; i <= 0x7F; ++i) {
			first_table[i] = OP_REL8;
		}
	...
	std::array<u16, 256> x64_disassembler::get_ext_table() {
		...
		ext_table[0x0B] = OP_0ARY;
		ext_table[0x18] = OP_FAKERX;
		ext_table[0x10] = OP_RM | OP_SSE;
		ext_table[0x11] = OP_MR | OP_SSE;
		ext_table[0x2E] = OP_RM | OP_SSE;
		ext_table[0x1F] = OP_RM;
		ext_table[0xAF] = OP_RM;

		for (int i = 0x51; i <= 0x5F; ++i) {
			ext_table[i] = OP_RM | OP_SSE;
		}
		...
		for (int i = 0xB6; i <= 0xB7; ++i) {
			ext_table[i] = OP_RM | OP_2DT;
		}

		for (int i = 0x80; i <= 0x8F; ++i) {
			ext_table[i] = OP_REL32;
		}
	...
	std::string x64_instruction::get_mnemonic() const {
		// cwd/cdq/cqo
		if (opcode == 0x99) {
			if (data_type_1 == word) {
				return "cwd";
