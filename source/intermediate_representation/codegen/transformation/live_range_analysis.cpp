...
namespace ir {
	void determine_live_ranges(codegen_context& context) {
		...
		for(const u64 block_order : context.basic_block_order) {
			auto target = context.work_list->items[block_order];
		...
		if (context.first) {
			...
			// initial label
			auto basic_block = context.work_list->items.front();
		...
		const u64 item_base = context.work_list->items.size();
		...
		while (context.work_list->items.size() > item_base) {
			handle<node> basic_block = context.work_list->items.back();
			context.work_list->items.pop_back();
