...
namespace sigma {
	...
	public:
		...
		enum class code : u32 {
			...
			UNEXPECTED_EMPTY_VOID_RET,
			UNEXPECTED_NON_EMPTY_VOID_RET,
			UNEXPECTED_TYPE,
			UNKNOWN_VARIABLE,
	...
	private:
		const static inline std::unordered_map<code, std::string> m_errors = {
			...
			{ code::UNEXPECTED_EMPTY_VOID_RET,           "invalid void return statement (expected 'ret value;', but got 'ret;' instead)"     },
			{ code::UNEXPECTED_NON_EMPTY_VOID_RET,       "invalid void return statement (expected 'ret;', but got 'ret value;' instead)"     },
