...
namespace ir {
	...
	public:
		...
		utility::outcome::result<code_buffer> compile(
			arch arch,
			system system
		);
		 ...
		 * \param path Path to the target 'dot' file
		 * \return Outcome (void).
		 */
		utility::outcome::result<void> print_node_graph(
			const filepath& path
		) const;
		 ...
		 */
		void create_ret(const std::vector<handle<node>>& values);
