...
namespace channel {
	acceptation_result codegen_visitor::visit_numerical_literal_node(
		...
		if (literal_type.get_pointer_level() > 0) {
			return std::unexpected(error::emit<4014>(node.get_declared_position()));
		}

		switch (literal_type.get_base()) {
			// signed integers
		case type::base::i8:
			return std::make_shared<value>("__i8", type(type::base::i8, 0), llvm::ConstantInt::get(m_context, llvm::APInt(8, std::stoll(node.get_value()), true)));
		case type::base::i16:
			return std::make_shared<value>("__i16", type(type::base::i16, 0), llvm::ConstantInt::get(m_context, llvm::APInt(16, std::stoll(node.get_value()), true)));
		case type::base::i32:
			return std::make_shared<value>("__i32", type(type::base::i32, 0), llvm::ConstantInt::get(m_context, llvm::APInt(32, std::stoll(node.get_value()), true)));
		case type::base::i64:
			return std::make_shared<value>("__i64", type(type::base::i64, 0), llvm::ConstantInt::get(m_context, llvm::APInt(64, std::stoll(node.get_value()), true)));
			// unsigned integers
		case type::base::u8:
			return std::make_shared<value>("__u8", type(type::base::u8, 0), llvm::ConstantInt::get(m_context, llvm::APInt(8, std::stoull(node.get_value()), false)));
		case type::base::u16:
			return std::make_shared<value>("__u16", type(type::base::u16, 0), llvm::ConstantInt::get(m_context, llvm::APInt(16, std::stoull(node.get_value()), false)));
		case type::base::u32:
			return std::make_shared<value>("__u32", type(type::base::u32, 0), llvm::ConstantInt::get(m_context, llvm::APInt(32, std::stoull(node.get_value()), false)));
		case type::base::u64:
			return std::make_shared<value>("__u64", type(type::base::u64, 0), llvm::ConstantInt::get(m_context, llvm::APInt(64, std::stoull(node.get_value()), false)));
			// floating point
		case type::base::f32:
